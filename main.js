const notas = {
    ...
    Tradicional: [[99, 100], [80, 89], [70, 79], [60, 69], [1, 60]],
    Point: [[12, 14], [9, 11], [6, 8], [3, 5], [1, 2]],
    Letter: ['A', 'B', 'C', 'D', 'E', 'Z']
...
let tipo = prompt('elige el tipo de nota 1.tradiciona 2.Puntos del 0-14 3.Letras')
...
let notaest = prompt(`Escribe la nota del estudiantes ${tipo == 1 ? notas.Tradicional.map(data => data.join().replace(',', ' - ')).join(' , ') : tipo == 2 ? notas.Point : tipo == 3 ? notas.Letter : errors('Tipo de nota no encontrada')}`)
...
switch (tipo) {
    case 1:
        ...
        console.log(tradicional(notas.Tradicional))
        break;
    case 2:
        notaest = parseInt(notaest)
        console.log(point(notas.Point))
        break
    case 3:
